# RoleSieve: выгрузка навыков из HH.ru в Excel
...
## Установка
1. Откройте терминал в корне репозитория RoleSieve
...
- `role_sieve/` — общая логика (API HH, ключевики, Excel, пайплайн).
- `hh_keyskills_export.py` — thin CLI над `role_sieve`.
- `web/app.py` — **FastAPI**: выгрузка в Excel по HTTP (для UI на Next.js).
- `frontend/` — **Next.js** (лендинг и `/analyze`).
- `oldFrontend/` — предыдущий UI (архив).

## HTTP API (FastAPI)

### Локальные переменные (`.env`)

Один раз скопируйте шаблон и заполните секреты — при каждом запуске терминала ничего вводить не нужно:

```powershell
copy .env.example .env
# отредактируйте .env: HH_TOKEN, ROLESIEVE_HH_USER_AGENT, при необходимости HH_CLIENT_ID/SECRET
```

Файл `.env` в корне репозитория подхватывается автоматически при старте API, CLI и `scripts/hh_app_token.py` (через `python-dotenv`). В git он не попадает.
...
### Токен приложения HH (client_id / client_secret)

`client_id` и `client_secret` **не подставляются в запросы к вакансиям** — из них один раз получают **`access_token`** (Bearer), его и передаёт RoleSieve.

**Способ 1 — в кабинете:** [dev.hh.ru/admin](https://dev.hh.ru/admin) → ваше приложение → скопировать `access_token` (появится после первой выдачи токена).

**Способ 2 — через OAuth (PowerShell):**

```powershell
$env:HH_CLIENT_ID="ваш_client_id"
$env:HH_CLIENT_SECRET="ваш_client_secret"
python scripts/hh_app_token.py
```

Скрипт выведет строку токена. Либо вручную:

```powershell
Invoke-RestMethod -Uri "https://hh.ru/oauth/token" -Method Post `
  -ContentType "application/x-www-form-urlencoded" `
  -Body "grant_type=client_credentials&client_id=$env:HH_CLIENT_ID&client_secret=$env:HH_CLIENT_SECRET"
```

В ответе поле `access_token` — это значение для `HH_TOKEN`. Токен приложения живёт ограниченное время (`expires_in` в ответе); при истечении запросите новый тем же способом.
...
`client_secret` храните только на сервере / в `.env`, не коммитьте в git.
...
| `ROLESIEVE_HH_USER_AGENT` | `HH-User-Agent` для API HH (формат `RoleSieve/1.0 (you@email.com)`). |
| `ROLESIEVE_REDIS_URL` | URL Redis для очереди RQ (опционально). |
...
3. Альтернатива: сборка из [Dockerfile](Dockerfile) в корне (`docker build -t rolesieve-api .`).
...
Локально после запуска API (см. секцию `HTTP API`):

```powershell
copy frontend\.env.example frontend\.env.local
```

В `frontend/.env.local` по умолчанию уже указан `NEXT_PUBLIC_API_URL=http://127.0.0.1:8000` (Next.js читает этот файл сам).
