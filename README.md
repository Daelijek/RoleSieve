...
- `newFrontend/hh-research-new/` — **Next.js** (лендинг и `/analyze`).
...
```powershell
cd newFrontend/hh-research-new
npm install
npm run dev
```
...
Локально после запуска API (см. секцию `HTTP API`) в `newFrontend/hh-research-new/.env.local` задайте:
