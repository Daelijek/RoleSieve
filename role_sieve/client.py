    ...
    for attempt in range(retries):
        try:
            ...
            if resp is not None and resp.status_code == 403:
                hint = (
                    "HH API: доступ запрещён (403). Зарегистрируйте приложение на "
                    "https://dev.hh.ru, укажите HH_USER_AGENT и Bearer-токен "
                    "(переменная HH_TOKEN на сервере или поле token в запросе)."
                )
                if not token:
                    hint += " Токен сейчас не передан."
